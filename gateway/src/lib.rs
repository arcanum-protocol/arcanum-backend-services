...
impl ServiceData for GatewayService {
    async fn run(self) -> anyhow::Result<()> {
        ...
        let price_fetcher_handle =
            tokio::spawn(price_fetcher::run(app_state.clone(), self.price_fetcher));
        ...
        tokio::select! {
            ...
            i = indexer_handle => i.unwrap().map_err(Into::into),
            p = price_fetcher_handle => p.unwrap().map_err(Into::into),
        }
    }
}
