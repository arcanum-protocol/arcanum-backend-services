use std::{collections::HashMap, env, iter::repeat, str::FromStr, sync::Arc, time::Duration};
...
async fn get_signed_price(
    ...
    let mp = storage
        .get(&params.multipool_id)
        .unwrap()
        .read()
        .await
        .clone();
    let contract = mp.contract_address();
    let price = mp
        .get_price(&contract)
        .unwrap()
        .not_older_than(180)
        .unwrap();
    let price = crypto::sign(contract, price, 42161, &signer);
